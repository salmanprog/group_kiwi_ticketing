...
document.addEventListener('DOMContentLoaded', function() {
    ...
    cabana.forEach(cab => {
        ...
        if(!grouped[cab.event_date]) grouped[cab.event_date] = {contracts: [], estimates: []};
    ...
    Object.keys(grouped).forEach(date => {
        ...
        const cabanaCount = grouped[date].cabana?.length || 0;
...
function openCabanaModal(cabana, date) {
    ...
    document.querySelector('#cabanaModal .modal-title').innerText = `Cabana on ${date}`;
