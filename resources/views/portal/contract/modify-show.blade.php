        ...
        <div class="row">
            <div class="col-md-10 offset-md-1">
                <div class="estimate-wrapper">
                    ...
                    {{-- Action Buttons --}}
                    @if ($record->is_client_approved == '0')
                        @if (Auth::user()->user_type == 'client')
                            <div class="action-buttons">
                                <form action="{{ route('estimates.accept', $record->id) }}" method="POST"
                                ...
                                <form action="{{ route('estimates.reject', $record->id) }}" method="POST"
                                    class="d-inline-block">
                                    <input type="hidden" name="slug" value="{{ $record->slug }}">
                                    @csrf
                    ...
                    @else
                        <div class="action-buttons">
                            <p class="text-muted mb-0">
                                ...
                                This Modification has already been {{ $record->is_client_approved == '1' ? 'accepted' : 'rejected' }}.
                            </p>
                        </div>
                    @endif
                    ...
                    <div class="estimate-header">
                        ...
                        <div class="estimate-meta">
                            <span class="estimate-number">#{{ $record->slug }}</span>
                            <div class="text-muted">
                                <i class="fas fa-calendar me-1"></i>Date:
                                {{ \Carbon\Carbon::parse($record->created_at)->format('F d, Y') }}<br>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <table class="product-table">
                            ...
                            <tbody>
                                @foreach ($record->contract_modified_items as $item)
                                    <tr>
                    ...
                    <table class="summary-table">
                        <tbody>
                            <tr>
                                <th>Subtotal</th>
                                <td>${{ number_format($record->contract_modified_items->sum('total_price'), 2) }}</td>
                            </tr>

                            {{-- Taxes --}}
                            @foreach ($record->contract_modified_items as $item)
                                <tr>
                                    <th>{{ $item->tax_name }} ({{ $item->tax_percent }}%)</th>
                                    <td>${{ number_format(($item->tax_percent / 100) * $item->total_price, 2) }}</td>
                                </tr>
                            @endforeach
                            ...
                            <tr>
                                <th>Total</th>
                                <td class="font-weight-bold">${{ number_format($record->contract_modified_items->sum('total_price'), 2) }}</td>
                            </tr>
                            <!--
                                <tr>
                                    <th>Amount Paid</th>
                                    <td class="text-success">${{ number_format($record->contract_modified_items->sum('total_price'), 2) }}</td>
                                </tr> -->
                        </tbody>
                    </table>
                    {{-- Notes--}}
               
                    @if ($record->installments && $record->installments->count() > 0)
                        <div class="installments-section mt-4">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($record->installments as $index => $installment)
                                            <tr>
                                                <td>#{{ $index + 1 }}</td>
