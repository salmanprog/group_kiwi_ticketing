...
<head>
    ...
    <!-- Load Poppins from Google Fonts with proper @import -->
    <style>
        /* Import Poppins font - this works better than link tag for PDFs */
        ...
        * {
        ...
        div, span, p, h1, h2, h3, h4, h5, h6, td, th, li, .card-value, .card-label, .date-value, .date-label {
            font-family: 'Poppins', 'Segoe UI', 'Helvetica Neue', Arial, sans-serif !important;
        }
        ...
        .summary-modern .amount {
            text-align: right;
            width: 25%;
            font-weight: 500;
        }

        .summary-modern .total-label {
            font-size: 14px;
            font-weight: 700;
        }

        .summary-modern .total-amount {
            font-size: 14px;
            font-weight: 700;
        }
...
<body>
    <!-- Your existing HTML body remains exactly the same -->
    <div class="watermark-number">#{{ $contract->slug ?? 'N/A' }}</div>
    ...
    <div class="grid-3">
        <div class="info-card">
            ...
            <div class="card-value">{{ $company->name ?? 'N/A' }}</div>
            <div class="card-sub">{{ $company->address ?? 'N/A' }}</div>
            <div class="card-sub">{{ $company->city ?? 'N/A' }}, {{ $company->zip ?? 'N/A' }},
                {{ $company->country ?? 'N/A' }}</div>
            <div class="card-sub">{{ $company->email ?? 'N/A' }}</div>
        </div>
                ...
                @foreach ($estimate->items as $item)
                    <tr data-id="{{ $item->id }}">
                        <td class="center">
                            ...
                            <br>
                            @if ($item->itemTaxes && $item->itemTaxes->count())
                                <small class="text-muted d-block" data-taxes='{!! json_encode(
                                    $item->itemTaxes->map(function ($tax) {
                                        return ['id' => $tax->id, 'name' => $tax->name, 'percent' => $tax->percentage];
                                    }),
                                ) !!}'>
                                    Apply Taxes:
                                    {{ $item->itemTaxes->pluck('name')->implode(', ') }}
    ...
    @if (!empty($estimate->note))
        <div style="font-family: 'Poppins', 'Open Sans', Arial, sans-serif; font-size:12px; color:#000; line-height:1.5;">
            {!! strip_tags($estimate->note, '<p><br><strong><em><ul><ol><li>') !!}
        </div>
    @else
        <div>N/A</div>
    ...
    <div class="footer-modern">
        CONTRACT #{{ $contract->slug ?? 'N/A' }} • GENERATED
        {{ optional($contract->created_at)->format('m/d/Y') ?? 'N/A' }}
    </div>
