...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'user_group_id','first_time_login', 'user_type', 'name', 'username', 'slug', 'email','auth0_id', 'mobile_no', 'password', 'image_url','blur_image','status',
    ...
    public static function getClientVerfiy($email)
    {
        $decrypted = Crypt::decrypt($email);
        $query = self::where('email',$decrypted)->first();
        if( isset($query->id) )
            return $query;
        else
            return false;
