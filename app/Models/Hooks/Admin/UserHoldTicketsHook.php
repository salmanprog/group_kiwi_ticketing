...
class UserHoldTicketsHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $slug=NULL) {
        //Your code here
        $query->select('user_hold_tickets.*', 'user_estimate.slug as estimate_slug');
        $query->join('user_estimate', 'user_estimate.id', '=', 'user_hold_tickets.estimate_id');
        $query->where('user_hold_tickets.auth_code', Auth::user()->auth_code);
    }
