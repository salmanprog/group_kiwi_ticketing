...
use Carbon\Carbon;
use App\Models\{Salesman, CompanyUser,User};
...
class SalesmanHook
{
   ...
   */
    ...
    {
        $query->select('users.*')->where('users.user_type', 'salesman')
            ->join('company_users', 'company_users.user_id', '=', 'users.id')
            ->where('company_users.user_type', 'salesman');
    ...
    */
    public function hook_before_add($request, &$postdata)
    {
        ...
        $postdata['user_group_id'] = 4;
        $postdata['password']  = Hash::make($postdata['password']);
        $postdata['user_type'] = 'salesman';
        if (!empty($postdata['image_url'])) {
            $postdata['image_url'] = uploadMedia('salesman', $postdata['image_url'], '50X50');
        }
    }
    ...
    */
    public function hook_after_add($request, $record)
    {
        ...
        $subject = "Salesman Register Confirmation";
        Mail::send('email.salesman_register', $data, function ($message) use ($record, $subject) {
