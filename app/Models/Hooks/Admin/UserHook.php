...
class UserHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $slug=NULL)
    {
        ...
        if( \Route::currentRouteName() == 'company-management.ajax-listing' ){
            $query->where('users.user_type','company');
        }

        if( \Route::currentRouteName() == 'manager-management.ajax-listing' ){
            $query->where('users.user_type','manager');
        }
